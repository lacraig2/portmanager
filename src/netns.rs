...
mod non_unix_impl {
    ...
    pub fn resolve_pid(_ns: &NsSpec) -> Result<Option<i32>> {
        Ok(None)
    }
    ...
    impl HelperPool {
        ...
        pub async fn connect(
            ...
            bail!(
                "namespace forwarding is not supported on this platform ({})",
                ns.to_wire()
            )
        }
    }
    ...
    pub fn run_helper() -> Result<()> {
        bail!("namespace helper is not supported on this platform")
    }
}
